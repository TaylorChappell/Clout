...
export default function AdminPage() {
  ...
  const deleteRevenue = async (statement: Statement) => {
    if (!window.confirm(`Delete the revenue report for ${statement.period}? This cannot be undone.`)) return;
    const busyKey = `delete-revenue-${statement.id}`;
    setBusy(busyKey);
    try {
      await api(`/v1/admin/revenue/${statement.id}`, { method: "DELETE" });
      setData((current) => ({ ...current, statements: current.statements.filter((item) => item.id !== statement.id) }));
      notify("Revenue statement deleted.");
    } catch (error) {
      notify(error instanceof Error ? error.message : "Statement could not be deleted.", "bad");
    } finally {
      setBusy("");
    }
  };
  ...
  return <main className={styles.adminPage}>
    ...
    <section className={styles.workspace}>
      ...
      {tab === "overview" && <div className={styles.view}><div className={styles.viewHeading}><span>Operations</span><h1>Everything behind the studio.</h1><p>Publish updates and weekly figures for the CLOUT ecosystem.</p></div><div className={styles.summaryGrid}><article><span>Published records</span><strong>{data.updates.length + data.statements.length}</strong><small>Updates and revenue statements</small></article><article><span>Launch platform</span><strong>Robinhood</strong><small>CLOUT coin ecosystem</small></article><article><span>Reward routes</span><strong>2</strong><small>CLOUT and verified Robux</small></article></div><div className={styles.operationGrid}><button type="button" onClick={() => setTab("updates")}><span>Publish</span><strong>New studio update</strong><small>Write the brief and full release notes.</small><i>↗</i></button><button type="button" onClick={() => setTab("revenue")}><span>Report</span><strong>Weekly revenue</strong><small>Add game, CLOUT and operating figures.</small><i>↗</i></button></div></div>}
